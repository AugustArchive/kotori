module.exports = class Command {
     ...
     * @returns {PromisedCommand} The promised command
     */
    async run(ctx) {
    ...
    toJSON() {
        const base = {};
        const props = ["command", "description", "usage", "category", "aliases", "guildOnly", "ownerOnly", "nsfw", "disabled", "throttle", "permissions"];
 ...
 * @prop {PermissionNode[]} [permissions=[]] The permissions in an array (returns an empty array if no permissions were added)
 * 
 * @typedef {import('../util/permissions').Permission} PermissionNode
 * @typedef {(client: import('../client')) => string} DescriptionSupplier
 * @typedef {void | string | import('@augu/eris').Embed} PromisedCommand
 */