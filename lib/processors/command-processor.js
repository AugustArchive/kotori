...
module.exports = class CommandProcessor {
    ...
    async process(msg) {
        ...
        const guildSettings = await this.client.getGuildSettings(msg.channel.guild.id);
        const userSettings  = await this.client.getUserSettings(msg.author.id);

        let prefix = null;
        const mention = new RegExp(`^<@!?${this.client.user.id}> `).exec(msg.content);
        const prefixes = [this.client.prefix, `${mention}`, guildSettings['prefix']];
